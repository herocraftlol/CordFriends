...
public class GuiBridgeListener implements Listener {
    ...
    public void onPluginMessage(PluginMessageEvent event) {
        ...
        } else if ("GUI_REQUEST".equals(action)) {
            handleGuiRequest(requester);
        }
    }

    /**
     * Un serveur backend demande l'ouverture du menu d'amis pour ce joueur (typiquement via
     * CrossFriendsSpigotAPI.openFriendsGui(player), utilisable par n'importe quel autre plugin
     * Spigot, par exemple lors d'un clic sur un item plutot que via la commande /friend gui).
     */
    private void handleGuiRequest(ProxiedPlayer requester) {
        GuiService.Result result = GuiService.openFriendsGui(plugin, requester);
        switch (result) {
            case NO_FRIENDS:
                requester.sendMessage(ChatColor.YELLOW + "Vous n'avez pas encore d'amis. Utilisez /friend add <joueur>.");
                break;
            case NOT_CONNECTED:
                requester.sendMessage(ChatColor.RED + "Impossible d'ouvrir l'interface pour le moment, reessayez dans un instant.");
                break;
            case OPENED:
                requester.sendMessage(ChatColor.GRAY + "Ouverture de l'interface d'amis...");
                break;
        }
    }
