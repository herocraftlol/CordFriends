# CrossFriends — module BungeeCord (proxy)

Plugin **BungeeCord** qui contient toute la logique : amis, blocage, messages privés et courrier hors-ligne, inter-serveur. C'est le seul module obligatoire. Le module `crossfriends-spigot` (interface graphique) est optionnel et vient en complément.
...
- La permission `crossfriends.bypass` contourne ces deux restrictions (utile pour le staff).
...
Le proxy seul ne peut pas ouvrir d'inventaire en jeu : c'est le rôle du module **crossfriends-spigot**, à installer en plus sur chaque serveur backend. `/friend gui` ouvre un inventaire listant vos amis (tête de joueur, statut, serveur). Clic gauche sur un ami en ligne = vous êtes transféré sur son serveur puis téléporté à côté de lui. Clic droit = un lien cliquable pré-remplit votre chat avec `/msg <ami> ` pour lui écrire directement. Voir le README du dossier `crossfriends-spigot` pour l'installation.

## Stockage des données

Un fichier JSON par joueur dans `plugins/CrossFriends/playerdata/<uuid>.json` (amis, demandes, blocages, courrier). Un fichier `names.json` retient la correspondance pseudo → UUID. Sauvegarde automatique toutes les 5 minutes et à l'arrêt du proxy.
...
3. Vérifiez dans la console que `CrossFriends active...` apparaît.
